...
def display_pic(scientific_selected):
	...
	try:
		wikipage = fetch_wiki_scientist(scientific_selected)
		st.image(wikipage.images[0], 
			caption=scientific_selected + 'Source: wikipedia')
	except:
		pass
		st.write('Photo à importer')
...
def fetch_csv_eleves_scientifiques():
	eleves_scientifiques_table = pd.read_csv('Cgenial_table_eleves_scientifiques.csv - Feuille 1.csv')
